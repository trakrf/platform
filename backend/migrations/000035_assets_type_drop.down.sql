...
ALTER TABLE assets ADD COLUMN type TEXT DEFAULT 'asset';
UPDATE assets SET type = 'asset' WHERE type IS NULL;
ALTER TABLE assets ALTER COLUMN type SET NOT NULL;
ALTER TABLE assets ADD CONSTRAINT assets_type_check CHECK (type IN ('asset', 'person', 'inventory'));

-- Restore the create_asset_with_tags signature with p_type.
