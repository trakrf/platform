...
func TestRemoveAssetTag_WrongAssetID_ReturnsFalse(t *testing.T) {
	...
	tag, err := store.AddTagToAsset(context.Background(), orgA, assetOwner.ID, shared.TagRequest{
		TagType: "epc",
		Value:   "EPC-OWNER",
	})
	require.NoError(t, err)
...
func TestRemoveLocationTag_CrossOrg_ReturnsFalse(t *testing.T) {
	...
	tag, err := store.AddTagToLocation(context.Background(), orgA, loc.ID, shared.TagRequest{
		TagType: "epc",
...
func TestRemoveLocationTag_WrongLocationID_ReturnsFalse(t *testing.T) {
	...
	tag, err := store.AddTagToLocation(context.Background(), orgA, locOwner.ID, shared.TagRequest{
		TagType: "epc",
		Value:   "EPC-LOC-OWNER",
	})
	require.NoError(t, err)
