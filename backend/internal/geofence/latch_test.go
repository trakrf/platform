...
func TestLatch_FirstSightFires(t *testing.T) {
	l := newLatch(time.Minute, 0, nil)
	defer l.Close()

	if !l.admit(keyFor(1, 2, "EPC"), time.Unix(100, 0)) {
...
func TestLatch_RepeatWithinTTLSuppressed(t *testing.T) {
	...
	if l.admit(key, base.Add(10*time.Second)) {
		t.Fatal("repeat within TTL must be suppressed")
	}
	if l.admit(key, base.Add(59*time.Second)) {
...
func TestLatch_DistinctKeysIndependent(t *testing.T) {
	...
	now := time.Unix(100, 0)
	if !l.admit(keyFor(1, 2, "A"), now) {
		t.Fatal("key A first sight must fire")
	}
	if !l.admit(keyFor(1, 2, "B"), now) {
		t.Fatal("key B first sight must fire (independent of A)")
	}
	if !l.admit(keyFor(1, 3, "A"), now) {
		t.Fatal("same epc at a different scan_point must fire (independent key)")
	}
}

func TestLatch_SweepEvictsIdle(t *testing.T) {
	clk := NewFakeClock(time.Unix(100, 0))
	l := newLatch(time.Minute, 0, clk) // sweepInterval 0 -> drive sweep() manually
	defer l.Close()

	l.admit(keyFor(1, 2, "A"), clk.Now())
	if l.size() != 1 {
