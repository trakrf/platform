...
// @Description
// @Description
// @Description Each row also carries the asset's current **dwell** — how long it has been at `location_id` without moving. `dwell_started_at` is the first observation of the asset at that location within its current unbroken run (a change of location starts a new run; a scan that resolved to no location also ends the previous one). `dwell_seconds` is `asset_last_seen - dwell_started_at`.
// @Description
// @Description Dwell is an **observed span, not a wall clock**: it is measured to `asset_last_seen`, so it stops growing once the asset stops being read rather than continuing to climb for an asset that has left. To decide whether a dwell is still current, compare `asset_last_seen` against your own staleness threshold. Both fields are always present. Dwell is derived from scan history at one-minute resolution and is reported conservatively — `dwell_started_at` is never earlier than the moment the asset was confirmed at that location. It reflects the underlying scan history, so it is still reported on rows whose `location_id` has been nulled out by soft-deletion or temporal validity.
// @Description
