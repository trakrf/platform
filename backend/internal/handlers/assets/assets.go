...
// @Description
// @Description  The `identifier` field is optional. Provide a value from your system of record
// @Description  (ERP, WMS, asset management) for natural-key joins, or omit it to receive a
// @Description  server-assigned identifier in the format `ASSET-NNNN` (per-organization sequence).
// @Description  A caller-supplied identifier that collides with an existing asset returns 409.
// @Description
...
func (handler *Handler) Create(w http.ResponseWriter, r *http.Request) {
	...
	// TRA-514: identifier is optional ("" → server autogens ASSET-NNNN), but
	// whitespace-only is ambiguous — surface the bad input instead of silently
	// substituting an autogenerated value.
	if request.Identifier != "" && strings.TrimSpace(request.Identifier) == "" {
