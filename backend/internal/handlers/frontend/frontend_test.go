...
func TestServeSPA_InjectsLabel(t *testing.T) {
	...
	// TRA-924: broker creds must never reach the browser again.
	if strings.Contains(body, "readerFeed") {
		t.Errorf("readerFeed config must be gone from injected app config:\n%s", body)
	}
}
