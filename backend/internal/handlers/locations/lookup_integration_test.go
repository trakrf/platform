...
func TestLookup_DuplicateExternalKey_Returns400(t *testing.T) {
	...
	cases := []struct {
		...
		{"two distinct values", "/api/v1/locations/lookup?external_key=wh-1&external_key=NOPE"},
		{"two identical values", "/api/v1/locations/lookup?external_key=wh-1&external_key=wh-1"},
	}
