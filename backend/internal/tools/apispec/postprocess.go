...
// [{APIKey: []}] (TRA-539 §2.6). This declares that every operation requires
// the APIKey scheme by default, so generated SDK clients authenticate every
// call automatically. Per-operation security overrides (e.g. security: [] on
// public or login endpoints) are respected by generators and are not disturbed
// here — we only set the document-level default if it is currently absent.
