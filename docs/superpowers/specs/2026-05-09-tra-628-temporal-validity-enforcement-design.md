...
- **Tag embed query drops the hardcoded `is_active = true` clause** and applies the temporal predicate instead. Tags are returned regardless of `is_active` in alignment with the assets/locations behavior; UI does not currently surface a tag-active filter.
...
|---|---|---|
| `backend/internal/storage/assets.go` | `buildAssetsWhere` (~line 848) | Append `temporallyEffective("a")` to default clauses. Also update the embedded tag subquery on the Q-search branch (~line 884) to drop hardcoded `i.is_active = true` and apply `temporallyEffective("i")`. |
| `backend/internal/storage/assets.go` | `getAssetWithLocationByID` (~line 510) | **No change.** Path-`{id}` override. |
| `backend/internal/storage/locations.go` | `buildLocationsWhere` (~line 890) | Append `temporallyEffective("l")`. Also audit any embedded tag subquery on the Q-search branch and apply the same change as above. |
