...
| 2026-08-31 | Final-fix D operations documentation | Clarified prominently that `Handler.RegisterRoutes` is defined/tested but not mounted by `serve.setupRouter`; the callback paths are intended future public, signature-protected endpoints rather than reachable production endpoints, and operators must wait for a durable consumer plus production root-router wiring before configuring or activating Twilio callback URLs. Documented pre-submit context cancellation and Twilio Go v1.30.9's lack of mid-request cancellation/deadline propagation through `CreateMessage`. |

### Current handoff

- Next task: independent final review of final-fixes A through D.
...
- Final-fix D boundary: the operations guide explicitly states that `Handler.RegisterRoutes` is defined/tested but `serve.setupRouter` does not mount it, so both callback paths are future public signature-protected endpoints and must not be configured or activated until durable consumer and production root-router wiring exist. It also documents pre-submit-only cancellation and the Twilio Go v1.30.9 mid-request context propagation limitation. No frontend, geofence, outbox, or suppression work was added or implied.
