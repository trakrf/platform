...
const CommissionKit: React.FC = () => {
  ...
  const canSave = label.trim() !== '' && members.length >= 2 && !isSaving;

  const handleSave = async () => {
    setError(null);
    try {
      const kit = await commission(buildCommissionRequest(label, tags, memberRoles));
    ...
    <div>
      ...
      <div className="mb-4 bg-white dark:bg-gray-800 rounded-lg shadow divide-y divide-gray-200 dark:divide-gray-700">
        <div className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400">
          Members ({members.length}) — scan with the trigger; location tags are excluded
        </div>
        {members.length === 0 && (
