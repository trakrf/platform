...
export default function SuperadminOrgsScreen() {
  ...
  useEffect(() => {
    ...
    orgsApi
      ...
      .catch((err: unknown) => {
        if (active) setError(extractErrorMessage(err, 'Failed to load organizations'));
      })
      .finally(() => {
    ...
    <div className="min-h-screen bg-gray-900 p-4">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          <a href="#home" className="text-gray-400 hover:text-gray-300 transition-colors" aria-label="Go home">
            <ArrowLeft className="w-5 h-5" />
          </a>
          <h1 className="text-2xl font-semibold text-white">All Organizations</h1>
        </div>
            ...
            <table className="w-full text-sm text-left text-gray-300">
              <thead className="text-xs uppercase text-gray-400 bg-gray-800">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3">Entitled</th>
              ...
              <tbody>
                {orgs.map((org) => (
                  <tr key={org.id} className="border-t border-gray-700 hover:bg-gray-800/50">
                    ...
                    <td className="px-4 py-3">{formatExpiry(org.subscription_expires_at)}</td>
                    <td className="px-4 py-3 text-right">{org.member_count}</td>
                  </tr>
                ))}
              </tbody>
            </table>
