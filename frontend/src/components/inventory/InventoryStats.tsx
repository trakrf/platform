...
export function InventoryStats({ stats, activeFilters, onToggleFilter, onClearFilters }: InventoryStatsProps) {
  return (
    <div className={`grid grid-cols-2 gap-2 md:gap-3 ${stats.hasReconciliation ? 'lg:grid-cols-5' : ''}`}>
      <button
        ...
        <div className="flex items-center justify-between">
          <div className="w-full">
            <div className="flex items-center mb-0.5 sm:mb-1 md:mb-2">
              ...
              <span className="text-blue-800 dark:text-blue-200 font-semibold text-[10px] xs:text-xs sm:text-sm md:text-base truncate">Scans</span>
            </div>
      ...
      <button
        onClick={() => onToggleFilter('Assets')}
        className={`bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-2 md:p-3 cursor-pointer transition-shadow text-left w-full ${
          activeFilters.has('Assets') ? 'ring-2 ring-purple-500 ring-offset-1 dark:ring-offset-gray-900' : ''
        }`}
        aria-pressed={activeFilters.has('Assets')}
      >
        <div className="flex items-center justify-between">
          <div className="w-full">
            <div className="flex items-center mb-0.5 sm:mb-1">
              <Save className="w-3.5 h-3.5 sm:w-4 sm:h-4 lg:w-5 lg:h-5 text-purple-600 mr-1 sm:mr-1.5 md:mr-2 flex-shrink-0" />
              <span className="text-purple-800 dark:text-purple-200 font-semibold text-[10px] xs:text-xs sm:text-sm lg:text-base truncate">Assets</span>
            </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="w-full">
                ...
                <div className="text-green-600 dark:text-green-400 text-[10px] xs:text-xs lg:text-sm truncate">Assets matched</div>
              </div>
            </div>
          </button>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="w-full">
                ...
                <div className="text-red-600 dark:text-red-400 text-[10px] xs:text-xs lg:text-sm truncate">Assets missing</div>
              </div>
            </div>
          </button>
            ...
            <div className="flex items-center justify-between">
              <div className="w-full">
                <div className="flex items-center mb-0.5 sm:mb-1">
                  ...
                  <span className="text-gray-800 dark:text-gray-200 font-semibold text-[10px] xs:text-xs sm:text-sm lg:text-base truncate">Extra</span>
                </div>
                <div className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-gray-800 dark:text-gray-200">{stats.notListed}</div>
                <div className="text-gray-600 dark:text-gray-400 text-[10px] xs:text-xs lg:text-sm truncate">Tags not in CSV</div>
              </div>
            </div>
          </button>
        </>
