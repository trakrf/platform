...
describe('CommandManager', () => {
  ...
  /**
   * One op code, two answer shapes.
   *
   * `0x8002` carries every RFID firmware operation, and on the way back it
   * carries two different things: a one-byte status for a register write —
   * `A7 B3 03 C2 82 9E 32 F1 80 02 00`, captured off the bench reader — and an
   * 8-byte REG_RESP for a register read, whose first byte is 0x70.
   *
   * `successByte` can only ever describe one of those. Judged by
   * `rawPayload[0] === 0x00`, a good register value reads as a failed command.
   * `isSuccess` lets the event that owns the op code own the question, and
   * every event without one keeps the `successByte` behaviour unchanged.
   */
  describe('isSuccess', () => {
    const dualShapeEvent: CS108Event = {
      ...testEvent,
      eventCode: 0x8002,
      name: 'DUAL_SHAPE_COMMAND',
      successByte: 0x00,
      isSuccess: (raw) => raw.length === 8 && raw[0] === 0x70,
    };

    function respondWith(event: CS108Event, rawPayload: Uint8Array): void {
      commandManager.handleCommandResponse({
        header: { prefix: 0xB3A7, messageLength: rawPayload.length, flags: 0, reserved: 0, crc: 0 },
    ...
    it('resolves a response its own predicate accepts, which successByte would reject', () => {
      const promise = commandManager.executeCommand(dualShapeEvent);
      const settled = promise.then(() => 'resolved', () => 'rejected');

      return wireHandedOver()
        .then(() => {
          respondWith(dualShapeEvent, new Uint8Array([0x70, 0x00, 0x00, 0x00, 0x2E, 0x60, 0x00, 0x02]));
          return vi.advanceTimersByTimeAsync(200);
        })
        .then(() => settled)
        .then((outcome) => expect(outcome).toBe('resolved'));
    });

    it('still rejects a response its predicate does not accept', () => {
      const promise = commandManager.executeCommand(dualShapeEvent);
      const settled = promise.then(() => 'resolved', () => 'rejected');

      return wireHandedOver()
        .then(() => {
          // Neither a REG_RESP nor a success status: 0xFF is the failure byte.
          respondWith(dualShapeEvent, new Uint8Array([0xFF]));
          return vi.advanceTimersByTimeAsync(200);
        })
        .then(() => settled)
        .then((outcome) => expect(outcome).toBe('rejected'));
    });

    it('leaves an event without a predicate on the successByte rule', () => {
      const promise = commandManager.executeCommand(testEvent);
      const settled = promise.then(() => 'resolved', () => 'rejected');

      return wireHandedOver()
        .then(() => {
          respondWith(testEvent, new Uint8Array([0x70, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]));
          return vi.advanceTimersByTimeAsync(200);
        })
        .then(() => settled)
        .then((outcome) => expect(outcome).toBe('rejected'));
