...
VALUES
    ...
    ((SELECT id FROM organizations WHERE domain = 'acme.com'),
     ...
     'admin', 'active'),
    -- TechStart user
    ((SELECT id FROM organizations WHERE domain = 'techstart.io'),
     (SELECT id FROM users WHERE email = 'admin@techstart.io'),
     'owner', 'active'),
    -- Research Lab user
    ((SELECT id FROM organizations WHERE domain = 'research-lab.edu'),
...
VALUES
    ((SELECT id FROM organizations WHERE domain = 'acme.com'), 'WAREHOUSE-1', 'Main Warehouse', 'Primary storage facility'),
    ((SELECT id FROM organizations WHERE domain = 'acme.com'), 'OFFICE-1', 'Head Office', 'Main office building'),
    ((SELECT id FROM organizations WHERE domain = 'techstart.io'), 'LAB-1', 'Development Lab', 'Main development space');

-- 5. Create scan devices
INSERT INTO scan_devices (org_id, identifier, name, type, description)
VALUES
    ((SELECT id FROM organizations WHERE domain = 'acme.com'), 'READER-001', 'Warehouse Entry Reader', 'rfid_reader', 'Main entrance RFID reader'),
    ((SELECT id FROM organizations WHERE domain = 'acme.com'), 'READER-002', 'Office Scanner', 'rfid_reader', 'Office entrance reader'),
    ((SELECT id FROM organizations WHERE domain = 'techstart.io'), 'READER-LAB-1', 'Lab Equipment Reader', 'rfid_reader', 'Lab equipment tracking');
...
VALUES
    ((SELECT id FROM organizations WHERE domain = 'acme.com'), 'PALLET-001', 'Pallet #001', 'inventory', 'Warehouse pallet'),
    ((SELECT id FROM organizations WHERE domain = 'acme.com'), 'LAPTOP-042', 'Laptop #042', 'equipment', 'Employee laptop'),
    ((SELECT id FROM organizations WHERE domain = 'techstart.io'), 'SCOPE-001', 'Oscilloscope #001', 'equipment', 'Lab equipment');
