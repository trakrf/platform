...
- TRA-682 BB28 fix wave (consolidated; pre-launch breaking changes):
  ...
  - **PATCH content-type tightened (RFC 7396 strict):** PATCH endpoints now reject `application/json` with 415 `unsupported_media_type`. Only `application/merge-patch+json` is accepted on PATCH; POST and PUT keep `application/json`. The 415 detail string is method-aware and names the correct content type per method.
